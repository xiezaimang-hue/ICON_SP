...
class StudioState:
    ...
    def destination_data(self, name: str):
        ...
        for page in project.get("pages", []):
            ...
            no_base_path = Path(page.get("prompt_no_base", ""))
            original_text = prompt_path.read_text(encoding="utf-8") if prompt_path.is_file() else ""
            no_base_text = no_base_path.read_text(encoding="utf-8") if no_base_path.is_file() else ""
            project_pages.append({
                **page,
                "prompt_text": original_text,
                "prompt_no_base_text": no_base_text,
                "prompt_variants": {
                    "original": original_text,
                    "no_base": no_base_text,
                },
        ...
        if ai_path and Path(ai_path).is_file():
            ...
            ai_generated_at = ai_report.get("generated_at", "")
...
<script>
const S={state:null,city:null,data:null,page:1,promptVariant:'original',filter:'pending',current:null,decision:'pending'};const $=s=>document.querySelector(s);const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const aiLabel=s=>({PASS:'通过',REVIEW:'待人工确认',FAIL:'图片不合格',REVIEW_ERROR:'系统错误',NOT_RUN:'未运行'})[s]||s;const promptVariantLabel=()=>S.promptVariant==='no_base'?'Prompt_无底座':'原版 Prompt';const api=async(path,opt={})=>{const r=await fetch(path,opt);const j=await r.json();if(!r.ok)throw Error(j.error||'请求失败');return j};const post=(path,data)=>api(path,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)});function toast(t){const e=$('#toast');e.textContent=t;e.classList.add('show');setTimeout(()=>e.classList.remove('show'),1800)}function asset(path){return path?'/asset?path='+encodeURIComponent(path):''}
