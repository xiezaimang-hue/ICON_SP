...
- 通过“导入素材”按钮手动上传外部 `.xlsx/.csv` 表格和多张网格图片
- 可选 OCR 去文字和 AI 整图初审
- 后台执行切图，界面不会在处理时卡死
...
1. 填写城市名称。
2. 选择一份 `.xlsx` 或 `.csv` 表格。表格需要包含 `POI / 景点 / 地标 / 名称` 之一；城市列支持 `城市 / 目的地 / destination / city`。
3. 选择该城市的全部4×4网格图片。图片按文件名自然顺序保存为 `batch1`、`batch2`……
4. 系统自动读取 POI、生成 `pois.json`，并验证图片数是否等于 `ceil(POI数量 / 16)`。

同名城市默认不会覆盖。勾选“备份并替换”后，旧输入会先移动到 `inputs/_backups/`，再导入新素材。上传只发生在本机 `127.0.0.1`，文件不会发送到外部服务。
