...
点击右上角“导入整张表”，选择 `.xlsx` 或 `.csv`。表格需要包含城市列和 `POI / 景点 / 地标 / 名称` 之一；可选的 `中文 / 中文名 / name_zh` 列会保留，并在人工评估时与英文名称同时显示。可选的 `英文 / 英文名 / name_en / prompt_name` 列会作为生图英文名；如果只有中文POI，系统会用本地词典自动生成 `prompt_name`。
...
每个PAGE同时提供 `原版 Prompt`、`Prompt_图标化` 和 `Prompt_本体强化` 三个版本。标题栏中的分段Tab只切换预览与复制内容；图标化版保存在 `prompts/Prompt_图标化/`，本体强化版保存在 `prompts/Prompt_本体强化/`。系统生成的Prompt会随规则升级自动刷新，但不会改变图片上传和切图对应关系。

三套Prompt中的POI标题和顺序都会优先使用英文 `prompt_name`，让外部图像模型按英文地点名生图；原始中文POI继续保留在项目数据里，用于人工评估对照和最终文件名。

`Prompt_图标化` 用于图标在移动端实际展示较小的场景：它不降低图像质量和渲染精度，但会要求模型减少微小细节、用2到4个大体块表达主体、强化单一大轮廓和色块，让结果更像清晰的App图标/玩具体块，而不是精细微缩模型。

`Prompt_本体强化` 用于需要更准确还原真实POI本体的场景：它延续图标化的低细节和大体块，但更强调标志性建筑/地标的真实颜色和主轮廓，用低细节块面表达建筑体量，减少“泛化成普通寺庙/普通塔/普通建筑”和过度精细的问题。

#### 2. 在外部软件生图并回填

进入“城市与Prompt”，选择PAGE并点击对应的“复制 PAGE N Prompt”，粘贴到外部生图网页。生成完成后进入“大图与切图”，在对应PAGE上传一张或多张候选大图。

- 每个PAGE最多10组候选。
- 可以每生成一张就上传一张，也可以一次上传多张。
- 组号由应用自动分配；删除候选组时，源图和切片会移入备份。

当前版本支持每个PAGE最多10组候选大图。点击PAGE旁的“上传候选”可一次加入多组；现有 `batchN` 会自动作为该PAGE的第1组保留。上传只保存源图，点击“切图当前城市”后统一处理所有待切候选组。

#### 3. 切图与质检

只要当前城市存在待处理候选组，“切图当前城市”就会启用。切图后进入人工评估；AI整图初审仍是可选项，默认关闭，每个候选大图消耗一次调用。

人工评估按POI展示所有候选切片。点击“设为最终”即完成该POI，也可改选或标记“需要重做”。所有POI选完后，“导出最终图片”会生成到 `outputs/<城市>/final/`，文件名采用 `城市英文_城市中文_POI英文_POI中文.png`，最终PNG最长边不超过100px。

如果某个POI的部分候选明显不可用，可以在详情区点击“删除”进入多选删除模式。被选中的候选会显示红色描边和叉，点击“确定删除”后，对应候选PNG会从候选文件夹中删除；如果它是当前最终选择，该POI会恢复为未选择。删除单张候选不会删除整组源图，也不会影响同组其它POI。

候选相关数据分别保存在 `inputs/<城市>/candidates.json`、`outputs/<城市>/candidate_manifest.json` 和 `outputs/<城市>/selections.json`。V1.3的 `manifest.json` 与 `cropped/` 会保留，不会被候选流程覆盖。

人工评估详情底部提供“搜索真实图片”按钮，会用当前城市、POI英文名和中文名打开 Bing 图片搜索，方便与切图结果并排核对。

人工评估工具栏的“导出审核总览图”会把当前城市全部POI、全部候选、中英文名称、人工结论、AI状态和备注排成一张1920px宽PNG长图。它不要求审核完成，也不会修改选择数据；文件保存在 `outputs/<城市>/review_boards/`，导出后会自动在新标签页打开。

AI审核通过本机已登录的 Codex CLI 调用，使用当前CLI支持的只读沙箱参数；调用参数或结构化结果异常时会标记 `REVIEW_ERROR`，但不会阻止切图输出。
